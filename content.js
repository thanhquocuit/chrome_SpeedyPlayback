(function () {
  function getVideo(doc) {
    ...
    for (i = 0; i < videoTags.length; i++)
      if (!videoTags[i].paused) return videoTags[i];
    for (i = 0; i < videoTags.length; i++)
      if (videoTags[i].played.length != 0) return videoTags[i];
    // Current playing audio
    for (i = 0; i < audioTags.length; i++)
      if (!audioTags[i].paused) return audioTags[i];
    for (i = 0; i < audioTags.length; i++)
      if (audioTags[i].played.length != 0) return audioTags[i];
    // The video or audio found
    if (videoTags.length == 1) return videoTags[0];
    if (audioTags.length == 1) return audioTags[0];
    ...
    if (request.msg == "speedy_change_speed") {
      ...
      if (video) {
        if (request.speed == 0.0) {
          ...
          return;
        } else if (request.speed < 0.5) request.speed = 0.5;
      ...
      video = getVideo();
      if (video) {
        if (!request.normal_rate) video.playbackRate = 0.5;
        if (video.currentTime - request.time > 0) {
          if (request.smooth_forward) {
            ...
            } else video.currentTime -= request.time;
          } else video.currentTime -= request.time;
        }
      ...
      for (i = 0; i < btn.length; i++) btn[i].click();
      btn = document.getElementsByClassName("ytp-ad-overlay-close-button");
      for (i = 0; i < btn.length; i++) btn[i].click();
      btn = document.getElementsByClassName("ytp-ad-skip-button-modern");
      for (i = 0; i < btn.length; i++) btn[i].click();
    } else if (
      request.msg == "speedy_skip_to_end" ||
  ...
  window.addEventListener("keydown", function (e) {
    chrome?.runtime?.sendMessage(
      { msg: "speedy_key_down", keyCode: e.keyCode },
      function (response) {
